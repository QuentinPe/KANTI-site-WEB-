...
import { FileText, Users, BookOpen, HelpCircle, Inbox, ArrowRight, TrendingUp, Clock } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const activity = [
    ...leads.slice(0, 4).map((l) => ({
      icon: Inbox,
      text: `Nouveau lead — ${l.nom} (${l.sujet ?? l.format ?? "contact"})`,
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {newLeads > 0 && (
        <Link to="/admin/leads"
          ...
          style={{ background: "hsl(0 65% 48% / 0.08)", border: "1px solid hsl(0 65% 48% / 0.18)" }}>
          <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: "hsl(0 65% 48%)" }} />
          <p className="text-[13px] font-medium" style={{ color: "hsl(0 60% 38%)" }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Leads totaux" value={leads.length} sub={`${todayLeads} aujourd'hui`} icon={Inbox} to="/admin/leads" color="hsl(0 65% 48%)" />
        <StatCard label="Cas clients" value={casClients.length} icon={Users} to="/admin/cas-clients" color="hsl(142 55% 38%)" />
        <StatCard label="Ressources PDF" value={ressources.length} icon={BookOpen} to="/admin/ressources" color="hsl(38 75% 42%)" />
      </div>
